...
GO_PACKAGES=./services/... ./indexer/... ./syscoin/... ./configuration/...
...
build-release:
	...
	docker build -t rosetta-syscoin:$(version) .;
	docker save rosetta-syscoin:$(version) | gzip > rosetta-syscoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/syscoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 8369:8369 rosetta-syscoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-syscoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/syscoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18369:18369 rosetta-syscoin:latest
