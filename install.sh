...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for coinbase/rosetta-syscoin 
...
execute() {
  ...
  docker tag "rosetta-syscoin:${TAG}" "rosetta-syscoin:latest"
  log_info "loaded rosetta-syscoin:${TAG} and tagged as rosetta-syscoin:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-syscoin
FORMAT=tar.gz
OWNER=coinbase
REPO="rosetta-syscoin"
PREFIX="$OWNER/$REPO"
