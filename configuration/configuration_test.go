	...
	tests := map[string]struct {
		...
		"all set (mainnet)": {
			...
			cfg: &Configuration{
				...
				Params:                 syscoin.MainnetParams,
				Currency:               syscoin.MainnetCurrency,
				GenesisBlockIdentifier: syscoin.MainnetGenesisBlockIdentifier,
				Port:                   1000,
		...
		"all set (testnet)": {
			...
			cfg: &Configuration{
				...
				Params:                 syscoin.TestnetParams,
				Currency:               syscoin.TestnetCurrency,
				GenesisBlockIdentifier: syscoin.TestnetGenesisBlockIdentifier,
				Port:                   1000,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.err != nil {
				...
				test.cfg.SyscoindPath = path.Join(newDir, "syscoind")
