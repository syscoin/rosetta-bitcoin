...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*syscoin.Client, *indexer.Indexer, error) {
	client := syscoin.NewClient(
		syscoin.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return syscoin.StartSyscoind(ctx, cfg.ConfigPath, g)
	})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-syscoin halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-syscoin sync failed", "error", err)
	}
}
