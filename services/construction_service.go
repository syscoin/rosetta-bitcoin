...
const (
	// bytesInKB is the number of bytes in a KB. In Syscoin, this is
...
func (s *ConstructionAPIService) estimateSize(operations []*types.Operation) float64 {
	size := syscoin.TransactionOverhead
	for _, operation := range operations {
		switch operation.Type {
		case syscoin.InputOpType:
			size += syscoin.InputSize
		case syscoin.OutputOpType:
			size += syscoin.OutputOverhead
			addr, err := btcutil.DecodeAddress(operation.Account.Address, s.config.Params)
			if err != nil {
				size += syscoin.P2PKHScriptPubkeySize
				continue
			}
...
func (s *ConstructionAPIService) ConstructionMetadata(
	...
	if feePerKB < syscoin.MinFeeRate {
		feePerKB = syscoin.MinFeeRate
	}

	// Calculated the estimated fee in Satoshis
	satoshisPerB := (feePerKB * float64(syscoin.SatoshisInSyscoin)) / bytesInKb
...
func (s *ConstructionAPIService) ConstructionPayloads(
	...
	for _, input := range matches[0].Operations {
		...
		transactionHash, index, err := syscoin.ParseCoinIdentifier(input.CoinChange.CoinIdentifier)
		if err != nil {
	...
	for i := range tx.TxIn {
		...
		class, _, err := syscoin.ParseSingleAddress(s.config.Params, script)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionCombine(
	...
	if err := json.Unmarshal(decodedTx, &unsigned); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal syscoin transaction", err),
		)
	}
	...
	for i := range tx.TxIn {
		...
		class, _, err := syscoin.ParseSingleAddress(s.config.Params, decodedScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionHash(
	...
	if err := json.Unmarshal(decodedTx, &signed); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal signed syscoin transaction", err),
		)
	}
...
func (s *ConstructionAPIService) parseUnsignedTransaction(
	...
	for i, output := range tx.TxOut {
		networkIndex := int64(i)
		_, addr, err := syscoin.ParseSingleAddress(s.config.Params, output.PkScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) parseSignedTransaction(
	...
	for i, input := range tx.TxIn {
		...
		_, addr, err := syscoin.ParseSingleAddress(s.config.Params, pkScript.Script())
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionSubmit(
	...
	if err != nil {
		return nil, wrapErr(ErrSyscoind, fmt.Errorf("%w unable to submit transaction", err))
	}
